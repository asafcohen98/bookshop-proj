...
function _createBooks() {
    ...
    if (!books || !books.length) {
        gNextId = 1
        books = [_createBook('Moby Dick', 19.90),
        _createBook('The Old Man And The Sea', 12.60),
        _createBook('Steve Jobs', 9.90)]
    }
    gBooks = books
...
function updateBookRate(bookId, diff) {
    ...
    if (diff === -1 && gBooks[bookIdx].rate === 0 || diff === 1 && gBooks[bookIdx].rate === 10) return
